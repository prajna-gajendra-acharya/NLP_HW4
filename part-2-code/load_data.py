...
PAD_IDX = TOKENIZER.pad_token_id                # usually 0 for T5
BOS_ID = TOKENIZER.pad_token_id                 # use pad as BOS, matching T5 pretraining

MAX_SRC_LEN = 256
MAX_TGT_LEN = 256   # increase to avoid truncating longer SQL queries
...
def normal_collate_fn(batch):
    ...
    # ---- decoder side (teacher forcing) ----
    dec_list = [item["dec_ids"] for item in batch]
    # build decoder_inputs by prepending BOS and dropping last to match target length
    dec_in_list = []
    dec_tgt_list = []
    for dec in dec_list:
        # Some datasets may have empty strings; guard just in case.
    ...
    # ---- first token for generation (greedy/beam) ----
